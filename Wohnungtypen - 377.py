...
bathtub_PossibleLayerNames = ["050 Möblierung", "050 Sanitäreinrichtung"]

zone_PossibleLayerNames = ["070 Raum"]
...
def CheckApartmentSize(area,roomNumber,bathtub, buffer):
    ...
    if(roomNumber  > 0 & roomNumber <=5):
        if((roomNumber-1)*15 + bathtub*5 + 47 + buffer > area): ### this is the general formula for Forderfahigkeit, works with any number of rooms
            return True
        else:
            return False
    else:
...
#check the length of Zone Numbers (they should have following system: 1.2.3.)
shortenedNumbers= []
for number in apartmentZoneNumbers:
    if(len(number)>6):
        shortenedNumbers.append(number[0:5])
    else:
        if len(number)==6:
            shortenedNumbers.append(number)
        else:
            print('Wrong zone number length(Zones)')
            shortenedNumbers.append(number)

apartmentZoneNumbers = shortenedNumbers
...
#check the length of Zone Numbers (they should have following system: 1.2.3.)
shortenedNumbers= []
for number in objectRelatedToZoneNumbers:
    if(len(number)>6):
        shortenedNumbers.append(number[0:5])
    else:
        if len(number)==6:
            ...
            print('Wrong zone number length(Objects)')
            shortenedNumbers.append(number)

objectRelatedToZoneNumbers = shortenedNumbers
...
for element in detailedValue[0].propertyDefinition.possibleEnumValues:
    if('Z' in detailedValue[0].propertyDefinition.possibleEnumValues[i].enumValue.displayValue):
